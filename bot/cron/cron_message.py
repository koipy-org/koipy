...
from bot.queue import MessageDeleteQueue, MessageEditQueue


async def cron_edit_message(app: Client, message_edit_queue: "MessageEditQueue"):
    edit_messages = []
    while True:
        try:
...
async def cron_delete_message(app: Client, message_delete_queue: "MessageDeleteQueue"):
    delete_messages = []
    while True:
        try:
